...
Return the variations as a JSON object with a key "queries" containing a list of 3 strings."""
...
try:
    ...
    query_variations = data['queries']
except json.JSONDecodeError:
    # Fallback: split by newlines and clean up
    lines = response.content.strip().split('\n')
    query_variations = [line.strip('- ').strip() for line in lines if line.strip()]
